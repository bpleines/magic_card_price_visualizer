...
class ScryfallScraper:
        ...
        for card_data_json in card_data_json_list:
            for card in card_data_json:
                ...
                try:
                    ...
                    card_attributes["release_year"] = int(
                        card.get("released_at").split("-")[0]
                    )
                    card_attributes["price"] = float(card.get("prices").get("usd"))
                    card_attributes["color"] = MTGCodes().encode_color(
                    ...
                    try:
                        ...
                        card_attributes[
                            "image"
                        ] = MTGCodes().encode_default_image_for_color(
                            card_attributes["color"]
                        )
                    cards.append(card_attributes)
                except TypeError:
                    logging.debug(
                        f"The card {card.get('name')} was missing an expected value. Skipping!"
                    )
                    continue
        return cards
    ...
    def generate_local_csv(self, set_or_color_code: str):
        ...
        if set_or_color_code in self.mtg_color_codes:
            csv_file_path = (
                f"{base_path}/magic_card_csv_files_by_color/{set_or_color_code}.csv"
            )
            card_data = self.query_card_color_data(set_or_color_code)
        else:
            csv_file_path = (
                f"{base_path}/magic_card_csv_files_by_set/{set_or_color_code}.csv"
            )
        ...
        if cards is not None:
            with open(csv_file_path, "a", encoding="utf-8") as local_csv_file:
                local_csv_file.write(
                    "name,cmc,release_year,price,color,image,type_line,artist\n"
                )
                for card in cards:
                    # TODO: break out into multiple lines
                    local_csv_file.write(
                        f"{card['name']},{card['cmc']},{card['release_year']},{card['price']},{card['color']},{card['image']},{card['type_line']},{card['artist']}\n"
                    )
